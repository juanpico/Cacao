...
# Importar función para realizar predicción de productividad ------------------
  ...
  setwd("/Users/juancamilopico/OneDrive - Universidad de los Andes/Uniandes/PG2/3. Modulo estimación")
  # Cargar el script con la función
  source("../3. Modulo estimación/predictor3.R")
...
# Preparar variables meteorológicas para predecir
  ...
  clima_acacias <- clima_acacias %>% dplyr::select(-Fecha)
  clima_villavicencio <- clima_villavicencio %>% dplyr::select(-Fecha)
  clima_cubarral <- clima_cubarral %>% dplyr::select(-Fecha)
...
boxplots['Ajustada'] <- prod_william[[1]]

boxplots_2 <- data.frame("Ajustada" = prod_fabio[[1]])
boxplots_2['Persona'] <- "Cultivo 1"
boxplots_2['Teórica'] <- prod_fabio[[2]]

boxplots_3 <- data.frame("Ajustada" = prod_silvia[[1]])
...
boxplots$tipo_prod <- factor(boxplots$tipo_prod, levels = c("Teórica", "Ajustada", "Reportada"))

colors <- c("Teórica" = "#a6cee3", "Ajustada" = "#1f78b4", "Reportada" = '#b2de8a')  
